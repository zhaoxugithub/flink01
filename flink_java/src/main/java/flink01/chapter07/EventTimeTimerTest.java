        ...
        stream.keyBy(t -> t.user)
               ...
               */.process(new KeyedProcessFunction<String, Event, String>() {
                  ...
                  public void processElement(Event value, KeyedProcessFunction<String, Event, String>.Context ctx, Collector<String> out) throws Exception {
                      ...
                      // 要用数据里面本身自带的时间戳
                      Long ctxStamp = ctx.timestamp();
                      out.collect(ctx.getCurrentKey() + " 数据到达,到达时间：" + new Timestamp(ctxStamp) + "水位线:" + new Timestamp(ctx.timerService()
                                                                                                                                       .currentWatermark()));
                      ctx.timerService()
                         .registerEventTimeTimer(ctxStamp + 10 * 1000);
                  }
                  ...
                  public void onTimer(long timestamp, KeyedProcessFunction<String, Event, String>.OnTimerContext ctx, Collector<String> out) throws Exception {
                      out.collect(ctx.getCurrentKey() + " 定时器触发，触发时间:" + new Timestamp(timestamp) + " 水位线:" + new Timestamp(ctx.timerService()
                                                                                                                                           .currentWatermark()));
                  }
              })
              .print();
